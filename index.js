...
const { handleReview } = require('./lib/commands/review');    // review 命令处理器
const { handlePush } = require('./lib/commands/push');        // push 命令处理器
...
${colors.yellow}用法：${colors.reset}
  ...
  sgc push               ${colors.gray}推送到所有远程仓库（多平台一键推送）${colors.reset}
  sgc push --force       ${colors.gray}强制推送到所有远程仓库${colors.reset}
  sgc init               ${colors.gray}初始化项目配置文件 (.sgcrc.json)${colors.reset}
  sgc review <分支>      ${colors.gray}代码审查（AI 分析分支间差异给出优化建议）${colors.reset}
...
${colors.yellow}示例：${colors.reset}
  ...
  sgc push               ${colors.gray}# 一键推送到所有远程仓库${colors.reset}
...
async function main() {
  ...
  const knownCommands = ['log', 'status', 'branch', 'hook', 'init', 'commit', 'review', 'push'];
  ...
  if (knownCommands.includes(command)) {
    switch (command) {
      ...
      case 'commit':
        ...
        break;
      case 'review':
        await handleReview(args.slice(1)); // 去掉 'review'，传剩余参数（如 'main'）
        break;
      case 'push':
        handlePush(args.slice(1));         // 去掉 'push'，传剩余参数（如 '--force'）
        break;
    }
  } else {
