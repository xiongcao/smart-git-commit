 ...
 */
/**
 * 调用 AI 接口根据代码变更内容生成 Conventional Commits 格式的提交信息
 * @param {string} diffDetail - Git diff 变更详情，超过 16000 字符会被截断以控制请求体大小
 * @param {string[]} files - 变更文件列表
 * @param {string} branch - 当前 Git 分支名称
 * @returns {Promise<string|null>} AI 生成的提交信息字符串；未配置 API Key 或生成失败时返回 null（降级为规则生成模式）
 */
