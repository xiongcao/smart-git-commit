...
async function handleCommit(args) {
  ...
  if (!autoCommit) {
    ...
    console.log(`  ${colors.bold}${colors.brightGreen}Enter${colors.reset}  ${colors.dim}- 确认并提交${colors.reset}`);
    ...
    // 编辑/修改类型后，重新展示最终提交信息并进入确认循环
    if (answer === 'e' || answer === 'v' || answer === 't') {
