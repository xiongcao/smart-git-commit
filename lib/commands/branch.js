...
async function handleBranch(args) {
  const config = loadConfig();  // 加载配置（获取 branchPrefixes 等）
...
async function createBranch(args, config) {
  const prefixes = config.branchPrefixes;  // 从配置中读取分支前缀映射
  ...
  const prefixList = Object.entries(prefixes).map(([type, prefix]) => ({
    value: prefix,
    name: `${type.padEnd(12)} → ${prefix}/`,  // padEnd(12) 对齐排版，让界面整齐
  }));
  ...
  runGit(`checkout -b ${fullName}`);  // git checkout -b feat/login：创建并切换到新分支
...
async function switchBranch() {
  const branches = getBranches();  // 获取所有分支列表
  ...
  runGit(`checkout ${selected}`);  // git checkout <branch>：切换分支
...
async function deleteBranch(args) {
  ...
  // --force 或 -f 参数：使用 -D 强制删除（即使分支未合并）
  const force = args.includes('--force') || args.includes('-f');
  const flag = force ? '-D' : '-d';  // -d 安全删除（仅已合并分支），-D 强制删除

  try {
    runGit(`branch ${flag} ${selected}`);
    console.log(`${colors.green}✅ 已删除分支: ${selected}${colors.reset}`);
  } catch (e) {
    // 非强制模式删除失败（可能分支未合并），询问用户是否强制删除
    if (!force) {
...
function listBranches() {
  const branches = getBranches();          // 获取所有分支（按最近提交时间排序）
  const currentBranch = getCurrentBranch(); // 获取当前分支名
