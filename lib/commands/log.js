...
function handleLog(args) {
  // 解析参数
  const useGraph = args.includes('--graph') || args.includes('-g');  // 图形化模式开关
  ...
  // 图形化模式：直接输出 git 原生的 graph 日志（ASCII 树形分支图）
  if (useGraph) {
    console.log(getGraphLog(count));
    return;
  }

  // 普通模式：获取提交记录，格式为 "%h %s"（短哈希 + 提交信息第一行）
  ...
  commits.forEach((line) => {
    ...
    if (match) {
      const hash = match[1];  // 短哈希（如 abc1234）
      const msg = match[2];   // 提交信息（如 feat(auth): 新增登录功能）

      // 解析 Conventional Commits 格式：type(scope): description
      const typeMatch = msg.match(/^(\w+)(\([^)]*\))?:\s*(.+)/);
      if (typeMatch) {
        const type = typeMatch[1];       // 类型：feat、fix 等
        const scope = typeMatch[2] || '';// scope：如 (auth)，可能为空
        const desc = typeMatch[3] || ''; // 描述：新增登录功能
  ...
  // 获取最近 50 条的类型统计（范围比显示的条数大，统计更有代表性）
  const stats = getCommitTypeStats(50);
  const total = Object.values(stats).reduce((a, b) => a + b, 0);  // 计算总数
