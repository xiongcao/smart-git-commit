...
function installHook() {
  ...
  if (fs.existsSync(hookPath)) {
    const existingContent = fs.readFileSync(hookPath, 'utf-8'); // 读取已有 hook 内容
    ...
    // 已有其他 hook（非 sgc 的），备份原文件（追加 .backup 后缀）
...
function uninstallHook() {
  ...
  if (fs.existsSync(backupPath)) {
    fs.copyFileSync(backupPath, hookPath);  // 恢复备份文件到原位置
    fs.unlinkSync(backupPath);              // 删除备份文件
    console.log(`${colors.green}✅ 已恢复原始 hook${colors.reset}`);
  } else {
    fs.unlinkSync(hookPath);                // 没有备份，直接删除 hook 文件
...
function checkHook() {
  ...
  if (fs.existsSync(hookPath)) {
    const content = fs.readFileSync(hookPath, 'utf-8'); // 读取 hook 文件内容

    // 检查内容是否包含 sgc 标记（通过 "Smart Git Commit" 字符串判断）
