...
const DEFAULT_CONFIG = {
  ...
  // API Key：AI 服务的认证密钥
  // 留空则从环境变量读取（OPENAI_API_KEY / DASHSCOPE_API_KEY）
  apiKey: '',
...
function loadConfig() {
  ...
  const globalConfigPath = path.join(
    process.env.HOME || process.env.USERPROFILE || '~',  // 跨平台获取用户目录
    '.sgcrc.json'
  );
  if (fs.existsSync(globalConfigPath)) {
    try {
      const globalConfig = JSON.parse(fs.readFileSync(globalConfigPath, 'utf-8'));
      Object.assign(config, globalConfig);               // 全局配置覆盖默认值
    } catch (e) {
      // JSON 解析失败时忽略，使用默认值继续
    }
  }
  ...
  try {
    ...
    if (repoRoot) {
      ...
      if (fs.existsSync(projectConfigPath)) {
        ...
        Object.assign(config, projectConfig);            // 项目配置覆盖全局和默认

        // typeRules 特殊处理：项目配置的规则追加到默认规则之后（而不是覆盖）
        // 这样用户添加的规则优先级更高（因为遍历时排在后面）
  ...
  // 配置文件中只能用字符串表示正则，这里统一转为真正的 RegExp
  config.typeRules = config.typeRules.map((rule) => ({
    ...rule,
    pattern: typeof rule.pattern === 'string'
      ? new RegExp(rule.pattern, 'i')    // 字符串 → RegExp，不区分大小写
      : rule.pattern,                     // 已经是 RegExp 则保持不变
  }));
 ...
 * JSON.stringify 第三个参数 2 表示缩进 2 空格，方便阅读
